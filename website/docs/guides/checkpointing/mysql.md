...
description: "Use MySQLCheckpointer for persistent agent state in CubeLoop with the MySQL backend."
---
...
Postgres (and unlike SQLite), CubeLoop **does not create these for you** —
it verifies on `__aenter__` that they exist with the expected
`schema_version`.

If they're missing, you get `CubeloopSchemaUninitialized`. If the version
doesn't match this CubeLoop release, you get `CubeloopSchemaMismatch`. A
...
- **`CubeloopSchemaUninitialized`** — Your DB is empty, your migrations
  didn't run, or the `cubepi_schema_version` table is malformed. Apply
  the host alembic upgrade first.
- **`CubeloopSchemaMismatch`** — You upgraded CubeLoop but didn't generate a
  new migration. Generate one, apply it, and CubeLoop will start.
...
- [Package README](https://github.com/cubeplexai/cubeloop/blob/main/cubeloop/checkpointer/mysql/README.md)
