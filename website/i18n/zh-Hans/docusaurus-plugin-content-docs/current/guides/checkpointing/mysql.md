---
title: MySQL 检查点
description: "在 CubeLoop 中使用 MySQLCheckpointer 通过 MySQL 后端实现 agent 状态持久化。"
---
...
Checkpointer 需要这些表：`cubepi_threads`、`cubepi_messages`、`cubepi_runs`、`cubepi_hitl_answers` 和 `cubepi_schema_version`。与 Postgres 一样（与 SQLite 不同），CubeLoop **不会为你创建这些表**——它会在 `__aenter__` 时验证这些表是否存在并具有预期的 `schema_version`。

若表不存在，会抛出 `CubeloopSchemaUninitialized`；若版本与当前 CubeLoop 版本不匹配，会抛出 `CubeloopSchemaMismatch`。生产数据库应由宿主应用的迁移系统（Alembic 等）管理，而非由第三方库管理。

### 通过 Alembic 初始化 Schema

CubeLoop 暴露了 SQLAlchemy 的 `MetaData`，供你的迁移脚本采用该 schema：

```python
# alembic/env.py
from cubeloop.checkpointer.mysql import cubeloop_metadata, EXPECTED_SCHEMA_VERSION
...
- **`payload` 是 msgpack 编码的 `model.model_dump(mode="json")`。** CubeLoop 在读取时重建 Pydantic 模型。
...
- **`CubeloopSchemaUninitialized`** —— 数据库为空、迁移未执行，或 `cubepi_schema_version` 表结构有误。请先应用宿主的 alembic upgrade。
- **`CubeloopSchemaMismatch`** —— 升级了 CubeLoop 但未生成新的迁移。生成迁移、应用后 CubeLoop 即可启动。
...
- [Package README](https://github.com/cubeplexai/cubeloop/blob/main/cubeloop/checkpointer/mysql/README.md) —— 与代码同处的完整宿主集成手册。
