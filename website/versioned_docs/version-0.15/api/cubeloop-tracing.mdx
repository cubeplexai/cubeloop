...
- `cubeloop.tags` — tuple of strings (OTel attribute type)
- one attribute per metadata key, namespaced under
  `cubeloop.metadata.*` (e.g. `metadata=\{"user_id": "u-42"\}` →
  `cubeloop.metadata.user_id = "u-42"`). The dedicated
  sub-namespace keeps recorder-owned schema keys
  (`cubeloop.run_id`, `cubeloop.turn.index`, …) safe from
...
[source](https://github.com/cubeplexai/cubeloop/blob/main/cubeloop/tracing/context.py#L51)
...
```python
JsonlSpanExporter(directory: str | os.PathLike[str] = './cubeloop-traces')
```
...
`cubeloop.run_id`) land together. The date used for the subdirectory is
...
[source](https://github.com/cubeplexai/cubeloop/blob/main/cubeloop/tracing/exporters/jsonl.py#L23)
...
[source](https://github.com/cubeplexai/cubeloop/blob/main/cubeloop/tracing/exporters/jsonl.py#L42)
...
[source](https://github.com/cubeplexai/cubeloop/blob/main/cubeloop/tracing/exporters/jsonl.py#L69)
...
[source](https://github.com/cubeplexai/cubeloop/blob/main/cubeloop/tracing/exporters/jsonl.py#L72)
...
[source](https://github.com/cubeplexai/cubeloop/blob/main/cubeloop/tracing/meter.py#L72)
...
[source](https://github.com/cubeplexai/cubeloop/blob/main/cubeloop/tracing/meter.py#L145)
...
[source](https://github.com/cubeplexai/cubeloop/blob/main/cubeloop/tracing/meter.py#L203)
...
[source](https://github.com/cubeplexai/cubeloop/blob/main/cubeloop/tracing/meter.py#L211)
...
Mirrors [`Tracer.attached`](./cubeloop-tracing#tracer-attached) — use both for the cleanest
end-to-end shutdown:
...
[source](https://github.com/cubeplexai/cubeloop/blob/main/cubeloop/tracing/meter.py#L218)
...
[source](https://github.com/cubeplexai/cubeloop/blob/main/cubeloop/tracing/schema.py#L13)
...
Attaches OTel-compatible tracing to a cubepi [`Agent`](./cubeloop-agent#agent).
...
[source](https://github.com/cubeplexai/cubeloop/blob/main/cubeloop/tracing/tracer.py#L115)
...
`cubeloop.mcp._tracing` so MCP CLIENT spans flow through
...
[source](https://github.com/cubeplexai/cubeloop/blob/main/cubeloop/tracing/tracer.py#L239)
...
[source](https://github.com/cubeplexai/cubeloop/blob/main/cubeloop/tracing/tracer.py#L334)
...
[source](https://github.com/cubeplexai/cubeloop/blob/main/cubeloop/tracing/tracer.py#L349)
...
with`` block. Combines with [`Tracer`](./cubeloop-tracing#tracer)'s own context
manager:
...
[source](https://github.com/cubeplexai/cubeloop/blob/main/cubeloop/tracing/tracer.py#L395)
...
`metadata` keys are stamped as `cubeloop.metadata.<key>` attributes on
the root span and are queryable with
`cubepi trace ls --meta <key>=<value>`.

`operation` is recorded in two places:

- `cubeloop.oneshot.operation` — for span introspection and dashboards.
- `cubeloop.metadata.oneshot_operation` — so the CLI `--meta` filter
  (which only reads `cubeloop.metadata.*` attributes) can reach it:
...
[source](https://github.com/cubeplexai/cubeloop/blob/main/cubeloop/tracing/tracer.py#L445)
...
Unlike [`Tracer.attached`](./cubeloop-tracing#tracer-attached), which surfaces flush failures to the caller,
...
[source](https://github.com/cubeplexai/cubeloop/blob/main/cubeloop/tracing/tracer.py#L776)
